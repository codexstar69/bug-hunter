<p align="center">
  <img src="https://raw.githubusercontent.com/codexstar69/bug-hunter/main/docs/images/hero.png" alt="Bug Hunter — Adversarial Bug Finding for Coding Agents. Pipeline: Triage → Recon → Hunters (Security, Logic) → Skeptics → Referee → Fixers → Verify. Compatible with all coding agent terminals and IDEs." width="720">
</p>
...
Bug Hunter is an open-source AI code review and security scanner for coding
agents. A Hunter finds bugs, a Skeptic tries to disprove them, and a Referee
delivers the final verdict. Only findings that survive this adversarial review
can enter the explicitly authorized fix pipeline.
...
> **Requirements:** Node.js 22+. The installed runtime has no npm dependencies.
>
> **Works with:** [Pi](https://github.com/mariozechner/pi-coding-agent),
> Claude Code, Codex, Cursor, Windsurf, Kiro, Copilot, OpenCode — or any AI
...
This release is a complete safety and reliability hardening pass across
mutation control, artifact contracts, orchestration, dependency analysis,
...
- **Broader regression proof.** The executable suite covers mutation safety,
  artifact schemas, orchestration, dependency scanning, installation, and
  release guards; run `pnpm test` for the current count and result.

<p align="center">
  <img src="https://raw.githubusercontent.com/codexstar69/bug-hunter/main/docs/images/2026-03-12-pr-review-flow.png" alt="PR review workflow banner — pull request scope, security checks, threat-model context, and final verdict in a clean product-style UI" width="100%">
</p>
...
```bash
/bug-hunter                        # single-pass scan, no edits
...
/bug-hunter --autonomous src/      # explicit unattended fixing
/bug-hunter --autonomous --auto-commit src/ # also create scoped commits
/bug-hunter --loop src/            # continue until queued coverage is complete
...
<p align="center">
  <img src="https://raw.githubusercontent.com/codexstar69/bug-hunter/main/docs/images/pipeline-overview.png" alt="Bug Hunter 8-stage adversarial pipeline architecture — Triage, Recon, Hunter deep scan, Documentation verification, Skeptic adversarial challenge, Referee independent verdict, Strategic fix plan, and Verification" width="100%">
</p>
...
<p align="center">
  <img src="https://raw.githubusercontent.com/codexstar69/bug-hunter/main/docs/images/adversarial-debate.png" alt="Adversarial debate workflow diagram — Hunter agent reports bug findings, Skeptic agent attempts disproof with counter-evidence, Referee agent delivers independent verdict with confidence scoring" width="100%">
</p>
...
<p align="center">
  <img src="https://raw.githubusercontent.com/codexstar69/bug-hunter/main/docs/images/2026-03-12-security-pack.png" alt="Bundled local security pack banner — Bug Hunter orchestrates commit security scan, security review, threat-model generation, and vulnerability validation" width="100%">
</p>
...
<p align="center">
  <img src="https://raw.githubusercontent.com/codexstar69/bug-hunter/main/docs/images/doc-verify-fix-plan.png" alt="Documentation verification workflow — agents check official library docs via Context Hub and Context7 API before making claims, plus strategic fix planning timeline with canary rollout and checkpoint verification" width="100%">
</p>
...
| **180+ files** | Large-codebase | Domain-scoped pipelines + boundary audits |

Normal use is one scan-only pass. Add `--loop` when the pipeline should keep
descending through CRITICAL → HIGH → MEDIUM → LOW until every queued source
file has a recorded outcome.
...
<p align="center">
  <img src="https://raw.githubusercontent.com/codexstar69/bug-hunter/main/docs/images/security-finding-card.png" alt="Enriched security finding card showing bug ID, severity badge, STRIDE and CWE classification, CVSS 3.1 score ring, reachability and exploitability ratings, and proof of concept payload" width="100%">
</p>
...
<p align="center">
  <img src="https://raw.githubusercontent.com/codexstar69/bug-hunter/main/docs/images/2026-03-12-fix-plan-rollout.png" alt="Strategic fix planning banner — strategy, confidence gating, canary rollout, verification, and rollback safety" width="100%">
</p>
...
<p align="center">
  <img src="https://raw.githubusercontent.com/codexstar69/bug-hunter/main/docs/images/2026-03-12-machine-readable-artifacts.png" alt="Machine-readable artifacts banner — findings, skeptic, referee, fix strategy, fix plan, and CI automation outputs" width="100%">
</p>

Every completed run produces validated machine-readable output at
`.bug-hunter/scan-report.json` for pipeline automation:

```json
{
  "schemaVersion": 1,
  "runId": "scan-2026-08-03-083000",
  "generatedAt": "2026-08-03T08:30:00.000Z",
  "mode": "local-sequential",
  "target": "src/",
  "filesScanned": 47,
  "threatModelLoaded": false,
  "dependencies": { "status": "not-applicable" },
  "counts": {
    "findings": 1,
    "confirmed": 1,
    "dismissed": 0,
...
| `hunter-findings.json` | Always | Canonical Hunter findings before review |
...
| `recon.json` | Always | Canonical tech stack, attack surface, and scan order |
| `scan-report.json` | Always | Joined final report with run metadata and complete counts |
...
**Source analysis:** TypeScript, JavaScript, Python, Go, Rust, Java, Kotlin,
Ruby, and PHP.

**Dependency reachability:** JavaScript/TypeScript package manifests. Other
ecosystems return `scanner-unsupported` until their parsers and fixtures exist;
they are never reported as clean by guesswork.
...
| Flag | Behavior |
|------|----------|
| *(no flags)* | Single-pass scan of the current directory; no code changes |
...
| `--fix` | Enable reviewed fixing after the scan |
...
| `--auto-commit` | Allow scoped commits during an explicit fix run |
...
| `--loop` | Iterate until every queued source file has a recorded outcome |
| `--no-loop` | Compatibility flag for the default single-pass behavior |
...
The repository ships with a Node.js regression suite covering orchestration,
schemas, PR scope, fix-plan authorization, lock behavior, worktree lifecycle,
installation, packaging, and security-skill routing. Run `pnpm test` for the
current executable count instead of relying on a copied documentation number.
...
│   └── dispatch.md                       #   Shared backend-neutral dispatch contract
│
├── prompts/                              # Generated compatibility copies
│
...
│   ├── scan-report.schema.json           #   Final joined scan-report schema
│   └── shared.schema.json                #   Shared definitions
│
├── skills/                               # Canonical role and security skills
│   ├── recon/
│   ├── hunter/
│   ├── skeptic/
│   ├── referee/
...
│   ├── run-bug-hunter.cjs                #   CLI parser and composition root
│   ├── process-runner.cjs                #   Bounded child-process lifecycle
│   ├── state-store.cjs                   #   Atomic run-state persistence
│   ├── artifact-planner.cjs              #   Canonical artifact planning
│   ├── chunk-scheduler.cjs               #   Retry-aware chunk scheduling
│   ├── bug-hunter-state.cjs              #   Persistent coverage state
...
│   └── tests/                            #   Source-only Node regression suite
│
├── templates/
